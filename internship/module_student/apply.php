      ...
      <div class="card">
        ...
        <?php
          ...
          else
          {
            $uid = $_GET['uid'];
            $student_id=$_SESSION['id'];
            $sql = "SELECT * FROM government_internship_apply WHERE student_uid = '$student_id' and internship_uid ='$uid'";
            $result = mysqli_query($conn, $sql);
            $resultChk = mysqli_num_rows($result);
            $company_id = $_GET['company_id'];
            if ($resultChk < 1) {
            ...
            $student_id=$_SESSION['id'];
                $applyId = "APPLY_".time().uniqid();
                $sql = "INSERT INTO government_internship_apply(uid, company_id, student_uid, internship_uid, status) VALUES ('$applyId', '$company_id', '$student_id', '$uid', '0');";
                mysqli_query($conn, $sql);
